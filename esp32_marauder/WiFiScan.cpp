...
{  
  ...
  else if (scan_mode == WIFI_ATTACK_SMALL_TIK)
    this->startWiFiAttacks(scan_mode, color, text_table1[52]);
...
void WiFiScan::StopScan(uint8_t scan_mode)
{
  ...
  (currentScanMode == WIFI_ATTACK_SMALL_TIK) ||
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if ((currentScanMode == WIFI_ATTACK_SMALL_TIK))
  {
    // Need this for loop because getTouch causes ~10ms delay
    // which makes beacon spam less effective
    for (int i = 0; i < 7; i++)
    {
      for (int x = 0; x < (sizeof(small_tik)/sizeof(char *)); x++)
      {
        broadcastSetSSID(currentTime, small_tik[x]);
      }
    }
